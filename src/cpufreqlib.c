...
void __fastcall fixed_loop(int count)
{
    __asm {
        mov edx, -1  // 将 -1 移动到 edx 寄存器
        mov eax, 2   // 将 2 移动到 eax 寄存器
        L2:           // 循环标签
        and ecx, edx // 将 ecx 与 edx 进行逻辑与操作
        sub ecx, eax // 从 ecx 中减去 eax
        jg L2         // 如果 ecx 大于 0，跳回 L2 继续循环
    }
}
